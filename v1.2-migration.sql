-- Hop Contract v1.2 cumulative migration
-- Safe to run on an existing v1.0 or v1.1 database.
-- Keeps recipe hops linked to Inventory by UUID and adds a supplier-receipts
-- cross-check field. Supplier receipts do NOT affect the contract calculation.
...
begin
  ...
  insert into public.app_settings (key, value)
  values ('app', jsonb_build_object('version','1.2'))
  on conflict (key) do update set value = excluded.value, updated_at = now();
end;
$$;
