-- Hop Contract v1.8 database migration
-- Run AFTER the v1.7 migration.
-- Adds the factual Previous Use (12m) value to frozen annual contract snapshots.
-- Existing finalised rows created before v1.8 default to 0 because that value was not previously snapshotted.

begin;

alter table public.contract_year_hops
  add column if not exists previous_use_12m_kg numeric(12,3) not null default 0
  check (previous_use_12m_kg >= 0);
...
where table_schema='public'
  ...
  and column_name='previous_use_12m_kg';
