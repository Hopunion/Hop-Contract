-- Hop Contract v1.13 database migration
-- Run after the working v1.10.3 schema/save hotfixes.
--
-- Adds exact-product contracting control and Northern/Southern hemisphere planning.
-- Existing HyperBoost products are excluded from contracting by default.
-- Existing business data is preserved.

begin;

alter table public.hop_inventory
  add column if not exists hemisphere text not null default 'Northern',
  add column if not exists contract_enabled boolean not null default true;
...
begin
  if not exists (
    ...
    where conrelid='public.hop_inventory'::regclass
      and conname='hop_inventory_hemisphere_check'
  ) then
    alter table public.hop_inventory
      add constraint hop_inventory_hemisphere_check
      check (hemisphere in ('Northern','Southern'));
  end if;
end $$;

create or replace function public.hop_product_hemisphere(product_name text)
returns text
language sql
immutable
...
as $$
  select case
    when lower(public.hop_product_variety(coalesce(product_name,''))) in ( 'nelson sauvin','nectaron','motueka','riwaka','rakau','wai-iti','waimea','kohatu', 'pacific jade','pacifica','pacific gem','southern cross','taiheke','moutere','superdelic', 'galaxy','vic secret','enigma','eclipse','ella','topaz' )
      then 'Southern'
    else 'Northern'
  end;
$$;

-- Seed known Southern varieties. Values remain editable in the app.
update public.hop_inventory
set hemisphere=public.hop_product_hemisphere(hop_name);

-- HyperBoost is still forecast from recipes, but is not contracted unless turned on.
update public.hop_inventory
set contract_enabled=false
where lower(coalesce(format,'')) in ('hyperboost','hyperboost oil')
   or hop_name ~* '\s+HyperBoost(?: Oil)?$';

-- Freeze the same metadata in annual contract history.
...
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.contract_year_hops'::regclass
      and conname='contract_year_hops_hemisphere_check'
  ) then
    alter table public.contract_year_hops
      add constraint contract_year_hops_hemisphere_check
      check (hemisphere in ('Northern','Southern'));
  end if;
end $$;

update public.contract_year_hops cyh
set
  hemisphere=coalesce(hi.hemisphere,public.hop_product_hemisphere(cyh.hop_name)),
  contract_enabled=case
    when hi.id is not null then hi.contract_enabled
    when cyh.hop_name ~* '\s+HyperBoost(?: Oil)?$' then false
    else true
  end
from public.hop_inventory hi
where hi.id=cyh.hop_inventory_id;

update public.contract_year_hops
set hemisphere=public.hop_product_hemisphere(hop_name)
where hop_inventory_id is null;

update public.contract_year_hops
set contract_enabled=false
where hop_name ~* '\s+HyperBoost(?: Oil)?$';

commit;

-- Cloud state read/write now persists Hemisphere and Contract?.
...
begin
  ...
  for v_inventory in select value from jsonb_array_elements(payload->'inventory') loop
    if nullif(trim(v_inventory->>'variety'),'') is not null then
      insert into public.hop_inventory (
        id, hop_name, variety, format, hemisphere, contract_enabled,
  ...
  insert into public.app_settings(key,value)
  values('app',jsonb_build_object('version','1.13'))
  on conflict(key) do update set value=excluded.value,updated_at=now();
end;
$$;
...
-- Finalised-year snapshots retain exact product contracting metadata.
...
begin
  ...
  for h in select value from jsonb_array_elements(coalesce(p_payload->'hops','[]'::jsonb)) loop
    ...
    insert into public.contract_year_hops(
      contract_year_id, hop_inventory_id, hop_name, hemisphere, contract_enabled,
...
-- Verification
select jsonb_build_object(
  'inventoryHemisphere', exists(select 1 from information_schema.columns where table_schema='public' and table_name='hop_inventory' and column_name='hemisphere'),
  'inventoryContractEnabled', exists(select 1 from information_schema.columns where table_schema='public' and table_name='hop_inventory' and column_name='contract_enabled'),
  'historicHemisphere', exists(select 1 from information_schema.columns where table_schema='public' and table_name='contract_year_hops' and column_name='hemisphere'),
  'historicContractEnabled', exists(select 1 from information_schema.columns where table_schema='public' and table_name='contract_year_hops' and column_name='contract_enabled'),
  'hyperBoostExcluded', not exists(select 1 from public.hop_inventory where (lower(coalesce(format,'')) in ('hyperboost','hyperboost oil') or hop_name ~* '\s+HyperBoost(?: Oil)?$') and contract_enabled=true)
) as v113_check;
