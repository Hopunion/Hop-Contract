-- Hop Contract v1.16 database migration
-- Adds additive forecast brew counts to live beers and immutable annual snapshots.
-- Safe whether or not the optional v1.15 contract-mix columns still exist.

begin;

alter table public.beers
  add column if not exists forecast_brews integer not null default 0;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.beers'::regclass
      and conname='beers_forecast_brews_check'
  ) then
    alter table public.beers
      add constraint beers_forecast_brews_check check (forecast_brews >= 0);
  end if;
end $$;

alter table public.contract_year_beers
  add column if not exists forecast_brews integer not null default 0;
...
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.contract_year_beers'::regclass
      and conname='contract_year_beers_forecast_brews_check'
  ) then
    alter table public.contract_year_beers
      add constraint contract_year_beers_forecast_brews_check check (forecast_brews >= 0);
  end if;
end $$;
  ...
  select jsonb_build_object(
    ...
    'beers', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'forecastBrews', coalesce(b.forecast_brews,0),
...
begin
  ...
  for v_beer in select value from jsonb_array_elements(payload->'beers') loop
    insert into public.beers (
      ...
      forecast_change_pct, forecast_brews, monthly_fixed_hl, one_off_hl, active, notes
    ) values (
      (v_beer->>'id')::uuid,
      ...
      greatest(coalesce((v_beer->>'forecastBrews')::integer,0),0),
    ...
    on conflict (id) do update set
      ...
      forecast_brews=excluded.forecast_brews,
  ...
  insert into public.app_settings(key,value)
  values('app',jsonb_build_object('version','1.16'))
  on conflict(key) do update set value=excluded.value,updated_at=now();
end;
$$;
  ...
  select case when auth.uid() is null then null else jsonb_build_object(
    ...
    'beers', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'forecastBrews', coalesce(cyb.forecast_brews,0),
...
begin
  ...
  for b in select value from jsonb_array_elements(coalesce(p_payload->'beers','[]'::jsonb)) loop
    ...
    insert into public.contract_year_beers(
      ...
      baseline_last12_hl, forecast_type, forecast_change_pct, forecast_brews,
      ...
      greatest(coalesce((b->>'forecastBrews')::integer,0),0),
...
select jsonb_build_object(
  'liveForecastBrews',
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='beers' and column_name='forecast_brews'),
  'historicForecastBrews',
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='contract_year_beers' and column_name='forecast_brews')
) as v116_check;
