-- Hop Contract v1.5 cumulative migration
-- Safe to run on an existing v1.0 or v1.1 database.
-- Keeps recipe hops linked to Inventory by UUID and retains the supplier-receipts cross-check and adds the previous/current
-- contract total for contract-to-contract comparison. Supplier receipts and
-- previous contract totals do NOT alter the forecast calculation.
...
alter table public.hop_inventory
  add column if not exists supplier_received_last_12m_kg numeric(12,3) not null default 0
  check (supplier_received_last_12m_kg >= 0);

alter table public.hop_inventory
  add column if not exists current_contract_total_kg numeric(12,3) not null default 0
  check (current_contract_total_kg >= 0);
...
as $$
  select case
    ...
    when coalesce(product_name,'') ~* '\s+Freshpak$' then 'Freshpak'
    when coalesce(product_name,'') ~* '\s+Leaf$' then 'Leaf'
...
as $$
  select trim(
    case public.hop_product_format(product_name)
      ...
      when 'Freshpak' then regexp_replace(coalesce(product_name,''), '\s+Freshpak$', '', 'i')
      when 'Leaf' then regexp_replace(coalesce(product_name,''), '\s+Leaf$', '', 'i')
...
-- Cloud load returns the exact inventory UUID on each recipe line and supplier receipt cross-check.
...
begin
  ...
  insert into public.app_settings (key, value)
  values ('app', jsonb_build_object('version','1.5'))
  on conflict (key) do update set value = excluded.value, updated_at = now();
end;
$$;
